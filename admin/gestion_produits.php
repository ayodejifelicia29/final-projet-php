...
if ($_POST) {
    ...
    if (!empty($_FILES) && !empty($_FILES["maPhoto"]["name"])) {
        ...
        $chemin_vers_la_photo_attribut_src = "img/" . $nomPhoto; //nom de variable$chemin_vers to take the pictures and go to base de donnees
        ...
        $dossier_sur_serveur_pour_enregistrer_photo = "" . $nomPhoto;
    ...
    if (isset($_POST["ajouterProduit"])) {
        ...
        $count = $pdo->exec("INSERT INTO produit (reference, categorie, titre, description, photo, prix, stock) VALUES(
                ...
                '$_POST[titre]',
                '$_POST[description]',
                '$chemin_vers_la_photo_attribut_src',
                '$_POST[prix]',
                '$_POST[stock]'
            )");
        ...
        $cheminPhoto = ($fileLoaded) ? $chemin_vers_la_photo_attribut_src : $_POST["prevPhoto"];
...
////////////////////////////////////////////
$stmt = $pdo->query("SELECT * FROM produit LIMIT $elementsPourLaPagination[premierArticle], 5"); //To show the limit number of the first 5 from the BDD)
...
<h1 class='mb-5 text-center'> La partie gestion de produit.</h1>

<!-- TABLE -->

<p class="bdd">Vos produits en BDD :</p>

<?php echo $content; ?>
<!-- The table to show the produts -->
<table class="table table-responsive mb-6">
        ...
        <?php foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $index => $produit) { ?>
            <tr>
                <?php foreach ($produit as $index => $valeur) {
                    if ($index == 'photo') { ?>
                        <td> <img style="width:50px" src="../<?php echo $valeur; ?>" alt=""> </td>
                    <?php } else { ?>
                ...
                <td> <a href="?action=suppression&id_produit=<?= $produit["id_produit"] ?>"> supprimer </a> </td>
            </tr>
        <?php } ?>

    </tbody>
</table>
<!-- Div of the nav to show the input of the producting -->
<div class="container">
    <div class="row">
        <div class="col-4 offset-md-2 mx-auto mt-5 mb-5 table-mobile-striped  ">
...
<p class="ajmodifier">Ajouter ou Modifier des produits :</p>
...
<form action="" method="POST" enctype="multipart/form-data">
    ...
    <div class="custom-file mb-12  p-2">
    ...
    <?php if (isset($_GET["action"]) && $_GET["action"] == "modification") { ?>
        <div class="text-center mb-4">
            <button type="submit" class="btn btn-secondary" style='background-color:#c09167' name="modifierProduit">Modifier un produit</button>
        </div>
    <?php } else { ?>
        <div class="text-center mb-3">
            <button type="submit" class="btn btn-secondary " style='background-color:#c09167' name="ajouterProduit">Ajouter un produit</button>
        </div>
    <?php } ?>
    </div>
