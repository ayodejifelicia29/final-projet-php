...
if ($_POST && isset($_POST["payer"])) {
    ...
    if (!isset($erreur)) { // Affiche l'erreur 
        ...
        // Récupérer l'id de la commande générée dans  la bdd
        ...
        for ($i = 0; $i < count($_SESSION["panier"]["id_produit"]); $i++) {
            ...
            //Execute an SQL statement and return the number of affected rows($pdo->exec)
      ...
      $pdo->exec('UPDATE produit SET stock = stock - " ' . $_SESSION['panier']['quantite'][$i] . ' " WHERE id_produit = "' . $_SESSION['panier']['id_produit'][$i] . '" ');
        ...
        $content .= "<div class=\"col-md-10 alert alert-success\" role=\"alert\">
...
if (isset($_POST["ajout_panier"])) { // execution apres envoi du formulaire
...
if (!empty($_SESSION["panier"]["id_produit"])) {
    $content .= "<div class='col-md-8 mb-5'>";
    $content .= "<a href='?action=viderPanier' class='badge badge-danger'> Vider le Panier </a>";
    $content .= "</div>";
}

// variable globale pour rendre le html dynamique($content)
// Début de la table panier
$content .= "<table class='table w-75 mx-auto'>
    <thead>
        <tr>
...
if (empty($_SESSION["panier"]["id_produit"])) {
    ...
    if (internauteEstConnecte()) {
        ...
        $content .= "<input type='submit' class='btn btn-outline-primary'  style='background-color:#c09167' name='payer' value='Payer'>";
        $content .= "</form>";
